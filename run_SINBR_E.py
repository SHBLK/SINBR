...
from SINBR_E import SINBR_main
import numpy as np
from warnings import warn

def run_SINBR(N_train,X_train,ind_cond,output_file,lr_N=0.001,lr_V=0.000001,size_NN=[240,300,240],tilde_M=50,num_post_samp=1000,opt_iters=6000):
    ...
    exit_code=SINBR_main(N_train,X_train,ind_cond,lr_N,lr_V,size_NN,tilde_M,num_post_samp,output_file,opt_iters)

    if exit_code==1:
        warn('The learning rate is too high. Trying a lower learning rate...')
        lr_N = min(lr_N,0.0001)
